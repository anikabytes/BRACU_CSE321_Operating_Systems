...
int main(){
	...
	printf("Enter the process name, arrival time, burst time & priority:-\n");
	for (int i=0; i<num; i++){
		printf("Process %d: ",i+1);
		scanf("%s %d %d %d",process[i].name, &process[i].arrival_time, &process[i].burst_time, &process[i].priority);
	...
	while (done < num){
		...
		for (int j=0; j<num; j++){
			if (process[j].arrival_time <= current_time && process[j].remaining_time > 0){
				if (check == -1 || process[check].priority > process[j].priority){
					check = j;
				}
			}
		}
	...
	for (int k = 0; k < num; k++) {
		printf("%s\t%d\t%d\t%d\t%d\t%d\t%d\n", process[k].name, process[k].arrival_time, process[k].burst_time, process[k].priority,process[k].completion_time,
		 	process[k].turnaround_time,process[k].waiting_time);
	}
}
